...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Select photons leaving the +X boundary
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% +X boundary is at x = x_dim = 100
on_xplus = abs(detp.p(:,1) - x_dim) < tol;

% Take a 1-mm-wide strip centered at Z = 50
z_middle = z_dim / 2;
dz = 1;

in_middle_strip = ...
    detp.p(:,3) >= z_middle - dz/2 & ...
    detp.p(:,3) <  z_middle + dz/2;

% Photon must leave +X AND be within middle Z strip
selected = on_xplus & in_middle_strip;


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Bin selected photons according to Y
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

y_edges = 0:1:y_dim;
y_centers = y_edges(1:end-1) + 0.5;

y_signal = zeros(1, y_dim);

for i = 1:y_dim

    in_y_bin = ...
        detp.p(:,2) >= y_edges(i) & ...
        detp.p(:,2) <  y_edges(i+1);

    y_signal(i) = sum(detw(selected & in_y_bin));
...
bar(y_centers, y_signal, 1);
hold on;
plot(y_centers, y_signal, '-');

xlabel('Y position (mm)');
ylabel('Escaping photon weight');

subtitle = sprintf( ...
    'Junge = %.3f, n = %.3f, mua = %.6g, mus = %.6g', ...
...
filename = sprintf( ...
    'ForwardDetectorTest_%.3f_n_%.3f_mua_%.6g_mus_%.6g.png', ...
    junge, index_of_ref, mua, mus);
...
fprintf('Photons through +X center strip: %d\n', sum(selected));
fprintf('Total weight through +X center strip: %.8g\n', ...
        sum(detw(selected)));